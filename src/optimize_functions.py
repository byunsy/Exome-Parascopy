...
def fopt(x,exome_data):
    ...
    for i in range(exome_data.n): 
        try: 
            ...
            a = math.log(p1)
            p = vec[i]/p1
            ll = math.log(p)*exome_data.counts[i] + math.log(1.0-p)*exome_data.ref_sums[i]
        except ValueError: ll =0
        total_ll += ll 
...
def pairwise_graph(data,thresh=5): ## calculate likelihoods for each edge
    graph = {}
    edges=[]
    for i in range(data.n): edges.append(0)
        ...
        for j in data.reference_sets[i]:
            ...
            for cn1 in range(1,data.refCN+3):
                for cn2 in range(1,data.refCN+3):
                    ...
                    if cn1 == cn2 and ll > best_triple[0]: best_triple[0] = ll
                    elif cn1 < cn2 and ll > best_triple[1]: best_triple[1] = ll
                    elif cn1 > cn2 and ll > best_triple[2] : best_triple[2] = ll
            d1 =best_triple[0]-best_triple[1]; d2 = best_triple[0]-best_triple[2]
            if d1 >= thresh and d2 >= thresh:     
                #print('edge',i,j,data.correlations[i][j])
                try: graph[i].append((min(d1,d2),j))
                except KeyError: graph[i] = [(min(d1,d2),j)]
                edges[i] +=1
            else: 
                d1 =best_triple[2]-best_triple[0]; d2 = best_triple[2]-best_triple[1]
