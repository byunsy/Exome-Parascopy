...
def filter_lmexons(allexons_df,file1=None):
    if file1 == None: return allexons_df
    filtered_exons = pd.read_csv(file1,sep='\t',header=None)
    exon_list= filtered_exons[filtered_exons.columns[3]].tolist()
    exon_index={}
    for index in exon_list: exon_index[index] = 1
    rows_to_remove = []
    for i in range(allexons_df.shape[0]):
        if allexons_df.iloc[i,3] not in exon_index and allexons_df.iloc[i,4] not in exon_index:            rows_to_remove.append(i)
...
def sample_readsums(count_matrix_file,exons_for_beta=10000,max_ref=15):
    ...
    for i in range(df1.shape[1]):
        ...
        for j,corr in corr_list[1:1+maxpairs]:
            ...
            if delta < 30: print('best',best_results[0],best_results[1],best_results[2],i,j,corr,'fit',binll,betabinll,delta,mean)
            else: print('BB',best_results[0],best_results[1],best_results[2],i,j,corr,'fit',binll,betabinll,delta,mean)
